NAME	= libft.a

CC		= gcc
CFLAGS	= -Wall -Wextra -Werror
LIB		= ar -rcs
LIB1	= ranlib
RM		= rm -f

INCLUDE	= libft.h

SRCS	=	ft_isalnum.c ft_isalpha.c ft_isascii.c ft_isdigit.c \
			ft_isprint.c ft_strlen.c ft_tolower.c ft_toupper.c \
			ft_strchr.c ft_strrchr.c ft_memset.c ft_bzero.c \
			ft_memcpy.c ft_memmove.c ft_memchr.c ft_memcmp.c \
			ft_strlcpy.c ft_calloc.c ft_strlcat.c ft_strncmp.c \
			ft_strnstr.c ft_strdup.c ft_atoi.c ft_putchar_fd.c \
			ft_putstr_fd.c ft_putnbr_fd.c ft_putendl_fd.c ft_split.c ft_substr.c \
			ft_strjoin.c ft_strmapi.c ft_strtrim.c ft_itoa.c \

OBJS =	${SRCS:.c=.o}

all:	${NAME}

${NAME}:	${OBJS}	${INCLUDE}
					${LIB} ${NAME} ${OBJS}
					${LIB1} ${NAME}

.c.o:
					${CC} ${CFLAGS} -c $< -o ${<:.c=.o}

clean:
					${RM} ${OBJS}

fclean:			clean
					${RM}    ${NAME}

re:				fclean all

.PHONY:			all clean fclean re